...
END;
    ELSE
    BEGIN
        SELECT @sql = '/*Scripting all user certificates' + CHAR(10) + '===================================================================================*/' + CHAR(13) + CHAR(13);
        SELECT @sql = @sql + 'CREATE CERTIFICATE ' + name + ' ENCRYPTION BY PASSWORD = ''P@ssw0rd1''
...
END;
    ...
    BEGIN
        ...
        DECLARE @uid INT;
        SELECT @sql = '/*Scripting all database users and schemas' + CHAR(10) + '===================================================================================' + CHAR(13) + 'Note: these are the users found in the database, but they may not all be valid, check them first*/' + CHAR(13) + CHAR(13);
        ...
        (
            ...
            BEGIN
                ...
                FROM #users;
                SELECT @sql = @sql + 'IF (SELECT name FROM sys.database_principals WHERE name = ''' + dp.name + ''') IS NULL' + CHAR(13) + 'BEGIN' + CHAR(13) + 'CREATE USER ' + QUOTENAME(dp.name) +
...
/*CASE
			...
			END +*/
                ...
                FROM sys.database_principals dp
                     LEFT OUTER JOIN sys.schemas sch ON dp.principal_id = sch.principal_id
                WHERE dp.principal_id = @uid
                      AND dp.TYPE IN('U', 'G', 'S')
...
(
    SELECT COUNT(*)
    FROM sys.database_principals dp
         INNER JOIN sys.certificates c ON dp.sid = c.sid
    WHERE dp.type = 'C'
          AND dp.principal_id > 4
) = 0
    BEGIN
...
END;
    ELSE
    BEGIN
        SELECT @sql = '/*Scripting all certificated database users' + CHAR(10) + '===================================================================================*/' + CHAR(13);
...
SELECT @sql = '/*Scripting all user schema permissions' + CHAR(10) + '===================================================================================*/' + CHAR(13);

--Script the permission grants on the schemas

SELECT @sql = @sql + CHAR(13) + dp.state_desc COLLATE latin1_general_ci_as + ' ' + dp.permission_name + ' ON ' + dp.class_desc + '::' + QUOTENAME(sch.name) + ' TO ' + QUOTENAME(dp2.name) + ' AS ' + QUOTENAME(dp3.name)
...
END;
    ELSE
    BEGIN
        SELECT @sql = '/*Scripting all database roles' + CHAR(10) + '===================================================================================*/' + CHAR(13);
        ...
        FROM sys.database_principals dp
             INNER JOIN sys.database_principals dp2 ON dp.owning_principal_id = dp2.principal_id
        WHERE dp.type = 'R'
...
(
    ...
    FROM sys.database_permissions dpm
         INNER JOIN sys.database_principals dp ON dpm.grantee_principal_id = dp.principal_id
...
END;
    ELSE
    BEGIN
        SELECT @sql = '/*Scripting all database and connection GRANTS' + CHAR(10) + '===================================================================================*/' + CHAR(13);
...
END;
    ELSE
    BEGIN
        SELECT @sql = '/*Scripting all database user role permissions' + CHAR(10) + '===================================================================================*/' + CHAR(13);
